...
import { hotelDummyData, roomsDummyData, userBookingsDummyData, vehicleData, vehicleBookingsData, ownerData } from '../../Assets/assets';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setRoomBookings(userBookingsDummyData);
        setVehicles(vehicleData);
        setVehicleBookings(vehicleBookingsData);
        setOwners(ownerData);
  ...
  const calculateRoomStats = () => {
    ...
    const averageRating = rooms.reduce((sum, room) => sum + (room.averageRating || 4.3), 0) / totalRooms;
  ...
  const calculateBookingStats = () => {
    ...
    const confirmedRoomBookings = roomBookings.filter(b => b.status === 'confirmed').length;
    const pendingRoomBookings = roomBookings.filter(b => b.status === 'pending').length;
    const cancelledRoomBookings = roomBookings.filter(b => b.status === 'cancelled').length;
    ...
    const pendingVehicleBookings = vehicleBookings.filter(b => b.booking_status === 'Pending').length;
    ...
    return {
      ...
      pendingBookings: pendingRoomBookings + pendingVehicleBookings,
      cancelledBookings: cancelledRoomBookings + cancelledVehicleBookings,
      ...
      vehicleStats: {
        ...
        pending: pendingVehicleBookings,
  ...
  const getNormalizedBookings = () => {
    return [
      ...roomBookings.map(booking => ({
        ...
        details: `${booking.room?.roomType || 'N/A'} (${booking.room?.hotel?.name || 'N/A'})`,
        customer: booking.guestName || 'N/A',
        days: booking.checkInDate && booking.checkOutDate
  ...
  const getRecentActivities = () => {
    return [
      ...roomBookings.slice(0, 3).map(booking => ({
        id: booking._id,
        type: 'room',
        title: `Room Booking ${booking.status}`,
        description: `Booking for ${booking.room?.roomType || 'N/A'} at ${booking.room?.hotel?.name || 'N/A'}`,
        date: booking.bookingDate,
                ...
                <tbody>
                  {roomBookings.slice(0, 5).map(booking => (
                    <tr key={booking._id}>
                      ...
                      <td>
                        <div>{booking.room?.roomType || 'N/A'}</div>
                        <div className="small-text">{booking.room?.hotel?.name || 'N/A'}</div>
                      </td>
      ...
      {activeTab === 'transport' && (
        <div className="transport-tab">
          ...
          <div className="chart-container">
            ...
            <div className="bar-chart">
              {['Bus', 'Car', 'Van', 'SUV', 'Truck', 'Motorcycle'].map(type => {
                  ...
                  {vehicleBookings.slice(0, 5).map(booking => (
                    <tr key={booking.booking_id}>
                      ...
                      <td>
                        ...
                        <div className="small-text">{booking.booking_days || 0} days</div>
                      </td>
                      <td>{formatCurrency(booking.totalPrice)}</td>
                      <td>
                    ...
                    <tr key={owner._id}>
                      <td>
                        <div className="owner-info">
                          <img 
                            src={owner.profile_pic || `https://ui-avatars.com/api/?name=${encodeURIComponent(owner.fullName || 'Owner')}&background=random&color=fff`} 
                      ...
                      <td>
                        <span className={`status-badge ${owner.isBlocked ? 'blocked' : 'active'}`}>
